...
export const BLOCKS = {
  pt: {
    ...
    planning: `<h2>A camada de planejamento · o que o MPC acrescenta, medido</h2>
    <p>Cada plano é um programa quadrático convexo sobre 96 quartos de hora: a dinâmica da bateria com perdas e um custo de desgaste por kWh ciclado, um modelo térmico de primeira ordem do prédio, a energia pedida por cada carro e seu horário de saída (como a ISO 15118 os passa ao carregador), a janela de redução anunciada pela distribuidora como restrição, os preços do dia seguinte e uma tarifa de demanda sobre o quarto de hora de maior compra. Ele é resolvido pelo Clarabel, um solver de pontos interiores escrito em Rust — no gateway, no estudo e nesta página. Os erros de previsão são tratados onde importam: a bateria só guarda reserva durante e pouco antes de uma redução esperada, o único momento em que a falta não pode ser comprada da rede.</p>
    <p style="margin-top:10px">Um estudo de Monte Carlo dá o número: o depósito em 30 dias de clima aleatório na primavera e 30 no inverno, com os preços reais desses dias, e a distribuidora reduzindo das 17:30 às 19:30 — o mesmo código, rodado 720 vezes.</p>
    ...
    <p style="margin-top:10px">O ganho vem dos preços, do pico e da inércia térmica do prédio, não de se proteger contra erros de previsão. A camada de tempo real absorve esses erros em segundos, e o §14a garante um piso. Com a redução passada para a tarde, a reserva probabilística custou 5 € por dia na primavera e só comprou alguns segundos a menos acima do piso. Um plano que ignora a tarifa de demanda empilha a recarga nas horas mais baratas e aumenta o pico; para um local comercial alemão que paga <i>Leistungspreis</i>, isso pode dar prejuízo.</p>
    <p style="margin-top:10px"><a href="${GH}/blob/main/docs/mpc.md">A formulação, as escolhas de projeto e o estudo completo →</a></p>`,
    countries: `<h2>Regras dos países · um controlador, três conjuntos de regras</h2>
    ...
    footer: `Local simulado, dados sintéticos: 120 kWp de solar no telhado, quatro carregadores de 22 kW, uma bomba de calor de 14 kW, uma bateria de 100 kWh. Regras dos países conforme publicadas; não é aconselhamento jurídico. O gateway também roda como programa nativo contra equipamentos Modbus TCP reais — veja o README. Feito por Michael Hiarley Silva Andrade · <a href="https://github.com/maycu-byte">github.com/maycu-byte</a>`,
  },
  de: {
    ...
    planning: `<h2>Die Planungsebene · was der MPC bringt, gemessen</h2>
    <p>Jeder Plan ist ein konvexes quadratisches Programm über 96 Viertelstunden: Batteriedynamik mit Verlusten und Alterungskosten je zyklierter kWh, ein thermisches Modell erster Ordnung des Gebäudes, der Energiebedarf und die Abfahrtszeit jedes Fahrzeugs (wie ISO 15118 sie an den Ladepunkt übergibt), das angekündigte Dimmfenster des Netzbetreibers als Nebenbedingung, Day-Ahead-Preise und ein Leistungspreis auf die höchste Bezugsviertelstunde. Gelöst wird es mit Clarabel, einem in Rust geschriebenen Innere-Punkte-Löser — im Gateway, in der Studie und auf dieser Seite. Prognosefehler werden dort behandelt, wo sie zählen: die Batterie hält nur während und kurz vor einer erwarteten Dimmung eine Reserve, dem einzigen Zeitpunkt, zu dem ein Fehlbetrag nicht aus dem Netz gekauft werden kann.</p>
    <p style="margin-top:10px">Eine Monte-Carlo-Studie beziffert das: das Depot an je 30 Tagen mit zufälligem Wetter im Frühling und im Winter, zu den echten Preisen dieser Tage, mit einer Dimmung von 17:30 bis 19:30 — derselbe Code, 720-mal ausgeführt.</p>
    <div class="scroll" style="margin-top:12px"><table>
    ...
    <p style="margin-top:10px">Der Nutzen kommt aus Preisen, der Lastspitze und der thermischen Masse des Gebäudes, nicht aus der Absicherung gegen Prognosefehler. Die Echtzeitschicht fängt diese Fehler in Sekunden ab, und §14a garantiert eine Mindestleistung. Mit der Dimmung am Nachmittag kostete die Chance-constrained-Reserve im Frühling 5 € am Tag und brachte nur wenige Sekunden weniger über der Mindestleistung. Ein Plan ohne Leistungspreis stapelt das Laden in die billigsten Stunden und erhöht die Spitze; für einen deutschen Gewerbestandort mit <i>Leistungspreis</i> kann das Geld kosten.</p>
    ...
    countries: `<h2>Länderregeln · ein Controller, drei Regelwerke</h2>
    <div class="scroll"><table>
    ...
    footer: `Simulierter Standort, synthetische Daten: 120 kWp PV auf dem Dach, vier 22-kW-Ladepunkte, eine 14-kW-Wärmepumpe, eine 100-kWh-Batterie. Länderregeln wie veröffentlicht; keine Rechtsberatung. Das Gateway läuft auch als natives Programm gegen echte Modbus-TCP-Geräte — siehe README. Erstellt von Michael Hiarley Silva Andrade · <a href="https://github.com/maycu-byte">github.com/maycu-byte</a>`,
  },
};
...
export const EXTRA = {
  pt: {
    ...
    "iec104 — controlled station, sans-IO link layer": "iec104 — estação controlada, enlace sem E/S", "planner — MPC: 24 h convex QP every 15 min": "planner — MPC: QP convexo de 24 h a cada 15 min",
  ...
  de: {
    ...
    "iec104 — controlled station, sans-IO link layer": "iec104 — Unterstation, Sans-IO-Verbindungsschicht", "planner — MPC: 24 h convex QP every 15 min": "planner — MPC: konvexes 24-h-QP alle 15 min",
...
Object.assign(BLOCKS.pt, {
  context: `<h2>O que está sendo simulado? · leia primeiro</h2>
    <div class="ctx-grid">
  ...
  calcGuide: `<div><b>1 · Descreva o bairro</b>Quantos depósitos dividem o transformador, quanto ele aguenta por depósito, quando as vans chegam, qual dia e quanto tempo dura a redução.</div>
      <div><b>2 · Escolha como os depósitos voltam</b>Todos de uma vez, devagar (rampa), depois de uma espera aleatória, em grupos — ou deixe cada depósito planejar pelo preço. Clique em <i>Calcular</i>, ou em <i>Comparar todas as opções</i> para testar todas.</div>
  ...
  data: `<h2>Fontes dos dados · o que é real e o que é modelado</h2>
    <div class="scroll"><table>
...
Object.assign(BLOCKS.de, {
  context: `<h2>Was wird simuliert? · bitte zuerst lesen</h2>
    <div class="ctx-grid">
      ...
      <div><span class="n">4</span><h3>Die Frage: was passiert, wenn die Reduzierung endet?</h3><p>Während der Reduzierung geraten die Lieferwagen in Rückstand. Endet sie, holen <b>alle Depots im Viertel</b> gleichzeitig auf, und die Spitze kann höher werden als ganz ohne Reduzierung — der <i>Nachholeffekt</i>. Der <a href="#calculator">Strang-Rechner</a> simuliert viele Depots zusammen, um den schonendsten Weg zurück zu finden.</p></div>
    </div>`,
  calcGuide: `<div><b>1 · Das Viertel beschreiben</b>Wie viele Depots sich den Trafo teilen, wie viel er je Depot trägt, wann die Lieferwagen ankommen, welcher Tag und wie lange die Reduzierung dauert.</div>
      <div><b>2 · Wählen, wie die Depots zurückkehren</b>Alle sofort, langsam (Rampe), nach einer zufälligen Wartezeit, in Gruppen — oder jedes Depot plant nach Preis. Klicken Sie auf <i>Berechnen</i> oder auf <i>Alle Optionen vergleichen</i>.</div>
  ...
  data: `<h2>Datenquellen · was echt ist und was modelliert</h2>
    <div class="scroll"><table>
