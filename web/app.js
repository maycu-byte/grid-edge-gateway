...
let demo, history, lastSample, speed = 300, playing = true, lastFrameT = null, state;
const faults = new Set();

function start(hour) {
  demo = new Demo(hour, SEED);
  history = [];
  lastSample = -Infinity;
  faults.clear();
...
function wire() {
  $("#dim-seg").addEventListener("click", (e) => {
    const b = e.target.closest("button"); if (!b) return;
    demo.command_dim(b.dataset.v === "on");
    press("#dim-seg", b.dataset.v);
    tick(0);
  });
  $("#feed-seg").addEventListener("click", (e) => {
    const b = e.target.closest("button"); if (!b) return;
    demo.command_feed_in(Number(b.dataset.v));
    press("#feed-seg", b.dataset.v);
    tick(0);
  });
  $("#speed-seg").addEventListener("click", (e) => {
    const b = e.target.closest("button"); if (!b) return;
    speed = Number(b.dataset.v);
    press("#speed-seg", b.dataset.v);
  });
  $("#scene-seg").addEventListener("click", (e) => {
    const b = e.target.closest("button"); if (!b) return;
    start(Number(b.dataset.v));
  });
  ...
  $("#hex").addEventListener("change", () => $("#log").classList.toggle("showhex", $("#hex").checked));
  const svg = $("#chart");
...
function tick(simSeconds) {
  ...
  if (state.t_s - lastSample >= SAMPLE_S || history.length === 0) {
    ...
    history.push({
      ...
      pv: state.pv_kw,
      steuve: state.steuve_kw,
      budget: state.dim_14a || state.mode === "releasing" ? state.steuve_budget_kw : null,
      exportLimit: state.feed_in_limit_pct < 100 ? -state.allowed_export_kw : null,
    });
  }
...
function renderMode() {
  const m = {
    normal: ["✓", "var(--good)", "Normal operation", state.feed_in_limit_pct < 100
      ? `DSO feed-in limit ${state.feed_in_limit_pct}%: export held at ${state.allowed_export_kw.toFixed(0)} kW, the depot's own load uses the rest of the PV.`
      : "No DSO command in force. Chargers and heat pump run at full power."],
    dimmed: ["↓", "var(--dso)", "§14a dimming active", `Controllable devices may draw ${kw(state.steuve_budget_kw)} from the grid: Pmin plus PV surplus, minus a 0.3 kW margin.`],
    releasing: ["↗", "var(--warn)", "Gradual release", "The DSO lifted the dimming. Power returns over 5 minutes so the feeder does not see a step."],
  }[state.mode];
  const fb = state.fallbacks.length ? `<br><span style="color:var(--critical)">Fallback: ${state.fallbacks.join(", ")}</span>` : "";
  $("#mode").innerHTML = `<div class="icon" style="background:${m[1]}">${m[0]}</div><div><strong>${m[2]}</strong><span>${m[3]}</span>${fb}</div>`;
}

function renderGauge() {
  const max = 110, now = state.steuve_grid_kw;
  const lim = state.dim_14a ? state.steuve_budget_kw : null;
  const g = $("#gauge");
...
function renderStats() {
  const stats = [
    ...
    [kw(state.pv_kw), `PV · inverter limit ${state.pv_limit_pct.toFixed(0)}%`],
    [kw(state.base_kw), "other load (office, cold store)"],
...
function renderDevices() {
  ...
  cards.push(`<div class="dev"><header><b>Heat pump</b><span class="pill ${hpStatus === "limited" ? "waiting" : hpStatus === "running" ? "charging" : "offline"}">${hpStatus}</span></header>
    ...
    <div class="sub">wants ${state.heat_pump_demand_kw.toFixed(1)} kW · 14 kW rated</div></div>`);
...
function drawChart() {
  ...
  const bands = [["budget", "§14a dimming"], ["exportLimit", "feed-in limit"]];
  ...
  // series
  const series = [["grid", "--s-grid"], ["pv", "--s-pv"], ["steuve", "--s-steuve"]];
  ...
  // DSO limits on top of the series (stepped, dashed)
  ...
  // now marker + direct labels at the head of each line
  const last = history[history.length - 1];
  if (last) {
    const labels = [["grid", "grid"], ["pv", "PV"], ["steuve", "steuVE"]]
      .map(([k, name]) => ({ name, v: last[k], y: y(last[k]) }))
      .sort((a, b) => a.y - b.y);
    for (let i = 1; i < labels.length; i++) labels[i].y = Math.max(labels[i].y, labels[i - 1].y + 14);
    for (const l of labels) {
      const lx = x(last.t) + 6;
      if (lx < W - 40) parts.push(`<text class="lbl" x="${lx}" y="${l.y + 4}">${l.name}</text>`);
    }
  }
  if (hoverT != null) {
...
function hover(e) {
  ...
  const lim = p.budget != null ? `<br>§14a budget <b>${kw(p.budget)}</b>` : "";
  const ex = p.exportLimit != null ? `<br>export limit <b>${kw(-p.exportLimit)}</b>` : "";
  tip.innerHTML = `${hh(p.t * 3600)}<br>grid <b>${kw(p.grid)}</b><br>PV <b>${kw(p.pv)}</b><br>chargers + heat pump <b>${kw(p.steuve)}</b>${lim}${ex}`;
...
// URL scenario for reproducible views, e.g. ?start=17&until=20&dim=17.75-19&feed=30@12-13
function scripted() {
  const q = new URLSearchParams(location.search);
  if (!q.has("start")) return false;
  ...
  let dimOn = false, feedOn = false, faulted = false;
