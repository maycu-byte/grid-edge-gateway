...
fn parse() -> Args {
    ...
    let usage = || -> ! {
        eprintln!(
            "usage: study [--seeds N] [--seasons spring,winter] [--strategies rules,mpc,...] [--hours H] [--dt S] [--out DIR]"
        );
        std::process::exit(2)
    };
...
fn run_one(season: Season, seed: u64, strategy: Strategy, hours: f64, dt_s: f64) -> Run {
    let mut cl = ClosedLoop::new(Scenario::study(season, seed), strategy, dt_s);
...
fn main() {
        ...
        for _ in 0..threads {
            s.spawn(|| {
                loop {
                    ...
                    let r = run_one(season, seed, strategy, a.hours, a.dt_s);
    ...
    md.push_str(&format!(
        "Generated by `cargo run --release -p closedloop --bin study -- --seeds {}`: {} seeds × {} seasons × {} strategies, {} h each from 06:00, control every {} s, MPC every 15 min (96 × 15 min horizon).\n\n",
        a.seeds,
        a.seeds,
        a.seasons.len(),
        a.strategies.len(),
    ...
    for &season in &a.seasons {
        ...
        md.push_str(&format!("### {}\n\n", capitalize(name)));
        md.push_str("| Strategy | Energy cost € | Battery ageing € | Total € | Δ total vs rules € | Discomfort K·h | EV energy missing kWh | Cars short | Dimming excess kWh | PV curtailed kWh | Solve ms (mean / max) |\n");
        md.push_str("|---|---|---|---|---|---|---|---|---|---|---|\n");
        ...
        for &strategy in &a.strategies {
            ...
            let total = col(&|r| r.metrics.energy_cost_eur + r.degradation_eur);
            ...
            let delta: Vec<f64> = rs
                ...
                .filter_map(|r| {
                    ...
                    Some(
                        (r.metrics.energy_cost_eur + r.degradation_eur)
                            - (b.metrics.energy_cost_eur + b.degradation_eur),
                    )
                })
                .collect();
            let d = mean_ci(&delta);
            ...
            let curt = col(&|r| r.curtailed_kwh);
    ...
    md.push_str("Values are means over the seeds ± the half-width of a 95% confidence interval of the mean. \"Δ total vs rules\" pairs each run with the rule-based run on the same weather.\n");
